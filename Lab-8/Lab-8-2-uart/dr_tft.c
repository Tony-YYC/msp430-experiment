...
#define tft_send_and_wait(x, y) \
    tft_SendCmd(x, y); \
    __delay_cycles(MCLK_FREQ / 1000);
#define tft_send_and_wait2(x, y, z) \
    tft_SendCmd(x, y); \
    __delay_cycles(MCLK_FREQ / 1000 * z);
...
void initTFT() {
    ...
    //配置与TFTLCD通信的串口
    UCB1CTL1 |= UCSWRST;
    UCB1CTL0 = UCCKPL + UCMSB + UCMST
        + UCSYNC; //下降沿变数据、上升沿采样；高位先；8位模式；主机；3线；同步
